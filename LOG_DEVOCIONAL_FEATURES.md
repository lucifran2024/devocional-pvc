...
- **Experiência:** cada Reel do Instagram ou vídeo do TikTok encontrado em uma anotação ganha sua própria caixa, com endereço, plataforma e botão **Transcrever este vídeo**. Se houver vários links, o usuário escolhe exatamente qual será transcrito. Ao criar ou editar uma nota com link, também pode usar **Salvar e transcrever**; o texto aparece na própria anotação com opções **Copiar texto** e **Salvar nesta anotação**.
...
- **Deploy:** frontend e Edge Function publicados; o workflow usa Supabase CLI `2.101.0` fixo para evitar falhas na resolução de `latest`.
- **Validação:** 5 testes de detecção/listagem de links, TypeScript, ESLint, build de produção e fluxo visual mobile (escolher entre dois vídeos → transcrever o link correto → criar link novo → salvar e transcrever) sem erros de console.
