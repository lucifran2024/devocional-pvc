...
// ===========================================
// DNA GERAÇÕES (HISTÓRICO RECENTE)
// ===========================================

export async function getDnaGeracoes(dias: number = 3): Promise<DnaGeracao[]> {
    console.log(`📜 [DNA-GEN] Buscando gerações dos últimos ${dias} dias...`);
    ...
    try {
        ...
        if (error) {
            console.error('❌ [DNA-GEN] Erro ao buscar:', error);
            return [];
        }

        return data as DnaGeracao[];
    } catch (e) {
        console.error('💥 [DNA-GEN] Exceção:', e);
        return [];
    }
}
...
export async function deleteDnaGeracao(id: number): Promise<boolean> {
    try {
        ...
        console.error('Erro ao deletar geração:', e);
        return false;
    }
}
...
export async function saveDnaGeracoes(
    ...
    try {
        ...
        const records = mensagens.map(msg => ({
            batch_id,
            texto_msg: msg,
        ...
        if (error) {
            console.error('Erro ao salvar gerações:', error);
            return false;
        }

        return true;
    } catch (e) {
        console.error('Exceção ao salvar gerações:', e);
        return false;
    }
}
